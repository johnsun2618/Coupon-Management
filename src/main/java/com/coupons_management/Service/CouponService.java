...
//        to check the expiration date when determining applicable coupons:
        for (Coupon coupon : couponRepository.findAll()) {
            ...
            // Ensure the cart has items and not empty
...
//            Applied if the cart's total amount meets a threshold.
...
//            Applied if the cart contains specific products.
...
//            Applied when certain products in specific quantities are purchased,
//            offering discounts in other products.
...
//                    iterates over the products defined in the "buyProducts" list of the coupon details and
//                    checks if they exist in the cart with the required quantity.
//                    If any product is missing or doesn't meet the quantity requirement,
...
//        applies a selected coupon to the cart, updating the total price or
//          applying product-specific discounts depending on the coupon type.
...
//            A percentage discount on the total cart value.
            case CART_WISE:
...
//            A percentage discount on specific products.
            case PRODUCT_WISE:
...
//            Discounted products when certain products are bought.
            case BxGy:
                if (details != null && details.containsKey("get_products") && details.containsKey("buy_products")) {
                    ...
                    // Check if the required quantity of "buy products" is present in the cart before applying the coupon.
                    // If even one product doesn't meet the required quantity, the coupon is not applied.
                    for (Map<String, Object> buyProduct : buyProducts) {
                        long buyProductId = ((Number) buyProduct.get("product_id")).longValue();
                        int requiredQuantity = ((Number) buyProduct.get("quantity")).intValue();
                        boolean productMatched = false;
